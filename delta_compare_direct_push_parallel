...
# ------------------------------
# Payload builder (Debezium envelope) — uses distinct BEFORE/AFTER type maps
# ------------------------------
...
def build_payload(
    ...
    op_type: str,
    schema: str,
    before_types: Dict[str, str],
    before_scales: Dict[str, Optional[int]],
    after_types: Dict[str, str],
    after_scales: Dict[str, Optional[int]],
    assume_tz: str = "UTC",
    ...
    norm_before = debezium_adaptive_normalize_record(
        before_dict, before_types, before_scales, assume_tz=assume_tz, schema=schema, table=table_name, side="before"
    )
    norm_after = debezium_adaptive_normalize_record(
        after_dict, after_types, after_scales, assume_tz=assume_tz, schema=schema, table=table_name, side="after"
    )
...
def _jsonb_build_object_chunks(alias, cols, typemap):
    ...
    return " ||\n ".join(chunk_exprs) if chunk_exprs else "to_jsonb(NULL)"
...
def build_fdw_delta_query_keys_only(
    schema_local: str, schema_remote: str, table_name: str, key_cols: list, typemap_union: dict
) -> str:
    def qident(name: str) -> str:
        return '"' + name.replace('"', '""') + '"'

    # include both key and non-key in objects to produce proper envelopes
    json_cols = list(typemap_union.keys())
    obj_local = _jsonb_build_object_chunks("l", json_cols, typemap_union)
    obj_remote = _jsonb_build_object_chunks("r", json_cols, typemap_union)
...
def build_fdw_delta_query_no_key(
    schema_local: str, schema_remote: str, table_name: str, typemap_union: dict
) -> str:
    def qident(name: str) -> str:
        return '"' + name.replace('"', '""') + '"'

    all_cols = list(typemap_union.keys())
    obj_local = _jsonb_build_object_chunks("l", all_cols, typemap_union)
    obj_remote = _jsonb_build_object_chunks("r", all_cols, typemap_union)
...
def build_fdw_delta_query(
    ...
    is_diff_parts = [_is_distinct_expr(c, typemap_union) for c in compare_cols]
...
# ------------------------------
# Column-type cache keyed by (schema, table) — thread-safe
# ------------------------------
_COLUMN_TYPE_CACHE: Dict[str, Tuple[Dict[str, str], Dict[str, Optional[int]]]] = {}
_CACHE_LOCK = Lock()
...
def get_column_types_cached(schema: str, table: str) -> Tuple[Dict[str, str], Dict[str, Optional[int]]]:
    key = _cache_key(schema, table)
    with _CACHE_LOCK:
        if key in _COLUMN_TYPE_CACHE:
            return _COLUMN_TYPE_CACHE[key]
    types, scales = get_column_types(schema, table)
    with _CACHE_LOCK:
        _COLUMN_TYPE_CACHE[key] = (types, scales)
    return types, scales


def discard_column_types(schema: str, table: str) -> None:
    key = _cache_key(schema, table)
    with _CACHE_LOCK:
...
def _log_temporal_map(schema: str, table: str, label: str, col_types: Dict[str, str], col_scales: Dict[str, Optional[int]], assume_tz: str) -> None:
    temporal_families = {"date", "time", "timetz", "timestamp", "timestamptz"}
    entries = []
    for c, t in col_types.items():
        tl = (t or "").lower()
        if tl in temporal_families:
            entries.append({"column": c, "type": tl, "scale": col_scales.get(c)})
    if entries:
        logging.info(
            f"Temporal columns [{label}] for {schema}.{table} (assume_tz={assume_tz}): "
            + ", ".join([f"{e['column']}:{e['type']}({e['scale']})" for e in entries])
        )
    else:
        logging.info(f"No temporal columns [{label}] for {schema}.{table}")
...
def send_to_eventhub(
    ...
    local_scales: Dict[str, Optional[int]],
    remote_types: Dict[str, str],
    remote_scales: Dict[str, Optional[int]],
    assume_tz: str = "UTC",
    ...
    # One-time logging per table to diagnose timestamp conversion maps
    _log_temporal_map(schema, table, "BEFORE(remote)", remote_types, remote_scales, assume_tz)
    _log_temporal_map(schema, table, "AFTER(local)",  local_types,  local_scales,  assume_tz)
    ...
    for _, row in df.iterrows():
        payload_obj = build_payload(
            ...
            op_type=row.get("op"),
            schema=schema,
            before_types=remote_types,
            before_scales=remote_scales,
            after_types=local_types,
            after_scales=local_scales,
            assume_tz=assume_tz,
...
def compare_table_fdw(table_name: str, schema_local: str, schema_remote: Optional[str] = None) -> str:
    try:
        ...
        # Column maps — cache per (schema, table), and compute both local and remote
        local_types, local_scales = get_column_types_cached(schema_local, table_name)
        remote_types, remote_scales = get_column_types_cached(schema_remote, table_name)

        # Build a union typemap used for SQL generation (e.g., xml casting).
        # If either side is xml, treat column as xml for expr/JSON building.
        typemap_union: Dict[str, str] = {}
        for c in set(list(local_types.keys()) + list(remote_types.keys())):
            lt = (local_types.get(c, "") or "").lower()
            rt = (remote_types.get(c, "") or "").lower()
            typemap_union[c] = "xml" if (lt == "xml" or rt == "xml") else (lt or rt or "text")

        if pk_cols:
            compare_cols = get_comparable_columns(db2_engine, schema_local, table_name, pk_cols)
            if not compare_cols:
                query = build_fdw_delta_query_keys_only(schema_local, schema_remote, table_name, pk_cols, typemap_union)
            else:
                query = build_fdw_delta_query(schema_local, schema_remote, table_name, pk_cols, compare_cols, typemap_union)
        else:
            query = build_fdw_delta_query_no_key(schema_local, schema_remote, table_name, typemap_union)
        ...
        if rows == 0:
            ...
            send_to_eventhub(
                ...
                remote_types,
                remote_scales,
                assume_tz=GLOBAL_ASSUME_TZ,
            )
            # discard cached types for this table to avoid stale reuse if tables are altered mid-run
            discard_column_types(schema_local, table_name)
            discard_column_types(schema_remote, table_name)
