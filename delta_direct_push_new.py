...
# Strict but lenient parser for timestamp WITHOUT time zone
# Accepts:
#   YYYY-MM-DD[ T]HH:MM:SS[.fraction up to 6]
# Ignores any trailing 'Z' or '+/-HH:MM' (they shouldn't be present for TIMESTAMP, but we strip if they are).
_TS_NAIVE_RE = re.compile(
    r"""^
        (\d{4})-(\d{2})-(\d{2})          # date
        [ T]
        (\d{2}):(\d{2}):(\d{2})          # time
        (?:\.(\d+))?                     # optional fractional seconds
        (?:Z|[+-]\d{2}:\d{2})?           # optional timezone tail (stripped)
        $
    """,
    re.VERBOSE,
)

def _parse_ts_naive_lenient(s: str) -> Optional[datetime]:
    s = s.strip()
    m = _TS_NAIVE_RE.match(s)
    if not m:
        return None
    y, mo, d, h, mi, se, frac = m.groups()
    # clip/pad to 6 digits for microseconds
    micro = 0
    if frac:
...
def _observed_frac_digits(val: object) -> int:
    """Count fractional digits present (0..6) to fallback when column scale is missing/invalid."""
    if isinstance(val, str):
        s = val.strip()
        # locate fraction after last '.' in the time part
        i = s.rfind(".")
        if i == -1:
            return 0
        j = i + 1
    ...
    if isinstance(val, datetime):
        return 6 if val.microsecond else 0
...
# Helper: safe stringify with truncation for logs
...
def debezium_adaptive_normalize_record(
    ...
    *,
    ...
    for col, val in record.items():
        ...
        if typ in ("time", "timetz", "timestamp", "timestamptz"):
            logging.debug(
                "Temporal conversion planned col=%s typ=%s scale=%s eff_scale=%s assume_tz=%s raw=%s",
                col, typ, scale, effective_scale, assume_tz, _short(val)
            )
        ...
        try:
            ...
            if typ == "timestamp":
                # Robust parse for variable-scale inputs (0..6), with lenient string handling.
                # 1) Strings → lenient parser (ignores stray 'Z'/'±HH:MM', accepts 0..6 fraction)
                # 2) datetime → drop tz if present
                if isinstance(val, str):
                    dt_naive = _parse_ts_naive_lenient(val)
                elif isinstance(val, datetime):
                    dt_naive = val if val.tzinfo is None else val.replace(tzinfo=None)
                else:
                    # preserve original for non-handled types
                    out[col] = val
                    # Optional: logging.debug("TIMESTAMP skip col=%s type=%s raw=%r", col, type(val).__name__, val)
                    continue

                if dt_naive is None:
                    # could not parse → preserve original (should be rare now)
                    out[col] = val
                ...
                eff = effective_scale
                if eff is None or eff < 0 or eff > 6:
                    obs = _observed_frac_digits(val)
                    eff = obs if obs > 0 else 6
